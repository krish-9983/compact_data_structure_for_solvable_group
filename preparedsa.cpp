...
<<<<<<< HEAD
    ...
    e.assign(orderGi, -1); sR.assign(orderGi, -1); sL.assign(orderGi, -1);

for (int i = 0; i < k; ++i) {
    int g0i = g0pow[i];
    for (int ni = 0; ni < (int)N_elems.size(); ni++) {
        int n_gid  = N_elems[ni];
        int g      = G_table[g0i][n_gid];      // g = g0^i * n  (global)
        int g_loc  = gid_to_Giidx[g];          // NEW: local Gi-index
        e[g_loc]   = i;
        sR[g_loc]  = ni;
        int inv_g0i    = inv[g0i];
        int nprime_gid = G_table[g][inv_g0i];
        sL[g_loc]      = gid_to_nidx[nprime_gid];
    }
}
return true;
=======
